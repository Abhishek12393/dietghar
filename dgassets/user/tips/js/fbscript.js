...
var callback = function (e){ 
    ...
    //	alert("No Flash Supported");
    	if('flipbuilder_hasMobile'=='flipbuilder_'+'hasMobile'){
    		delayToShowVersion("mobileTips")
    		// window.location = "mobileTips";
    ...
    //    if(BR.Platform.name == 'ios'||BR.Platform.name == 'android') window.location = "mobileTips";
...
var callback = function (e){ 
  ...
  if(!e.success || !e.ref){ //	alert("No Flash Supported");
    if(hasMobileVersion){
    	var pageIndex=getURLParam('pageIndex');
		  if(!pageIndex){
		      window.location = "mobileTips"+(window.location.hash?window.location.hash:'');
		   }else{
		   		window.location = "mobileTips#p="+pageIndex;
		  	}
	 		
		}else if(hasBasicHtmlVersion){
			 window.location ='files/basic-html/index.html';
		}else{
			document.write("Sorry,need flash player. <a href='http://www.adobe.com/go/getflashplayer'>Get Adobe Flash Player<\/a> it's possible to <a id='linkSEO' href='files/basic-html/index.html'>view a simplified version of the book on any device</a>, or you can view the mobile version <a href='mobileTips'> here </a>" ); 
		} 
    return false; 
  }  
};
