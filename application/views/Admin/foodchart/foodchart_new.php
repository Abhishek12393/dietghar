		...
		<div class="card">
			...
			<div class="table-responsive">
				...
				<?php if(isset($charttype) && $charttype == 'editchart'){
					...
					?><form method="post" action="<?=base_url('Admin/submit_food_chart_temp')?>" target="_blank"><?php
				}
				 ?>
				...
				<!-- <form method="post" action="<?=base_url('Admin/checkt')?>" target="_blank" enctype="multipart/form-data"> -->
					...
					<?php  if(isset($renewchart) && $renewchart == 1) {?>
							...
							<?php } ?>
					<?php  if(isset($entersameshift) && $entersameshift == 2) {?>
								<input type="hidden" name="entersameshift" value="<?=$entersameshift?>" >
								Mode: Blank Shift Entry 
					<?php } ?>
						...
						<tbody>
							<?php foreach ($message as $key => $value) {
								...
								<tr>
									...
									<?php if(in_array('Early Morning',$_POST['Placement']))
									{ ?>
										<td style="width: 12% ; <?php if($day%5 == 1 && isset($entersameshift) && $entersameshift == 2){ ?>background-color: darkslategray<?php } ?> " class="cursor-move" >
...
<?php if(in_array('Bed Time',$_POST['Placement']))
{ ?>
	<td style="width: 12%;<?php if($day% 5 == 1 && isset($entersameshift) && $entersameshift == 2 ){ ?>background-color: darkslategray<?php } ?>" class="<?php echo $type_class_bedtime;?>">
		<!-- <p class="badge badge-clr2">137</p> -->
		<div style="padding-top: 15%;">
			<p style="font-size: 14px; word-wrap: break-word;text-align: justify;">
