...
var ref =function (firebase){
    return {
        ...
        getUserAddconnRef : function(userId){
            return firebase.database().ref().child(table.users).child(userId)
            .child(user.connections);
        },
        getchatroomInfoRef : function(userId){
            return firebase.database().ref().child(table.chatRoom).child(userId)
            .child(chatRoom.roomInfo);
        },
        // user reference end 
...
var userRepo =function(firebase){
    return {
        addUser: function(userInfoModel){
               var userInfoRef=(ref(firebase)).getUserInfoRef(userInfoModel.uniqueId); 
...
var chatroomRepo =function(firebase){
    return {
        addUser: function(userInfoModel){
               var chatInfoRef=(ref(firebase)).getchatroomInfoRef(userInfoModel.uniqueId); 
               //var strTime = (new Date()).getTime()+"";
                //userInfoModel.timespam= strTime;
               chatInfoRef.set(userInfoModel);
        },
        addConnection:function(userId, connectionId){
           var userAddConnRef=(ref(firebase)).getUserAddconnRef(userId); 
           userAddConnRef.child(connectionId).set(true);
